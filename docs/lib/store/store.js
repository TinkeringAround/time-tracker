import {calcTimeCredits, separateDate, toDateString} from "../date.js";
import {
...
export class Store {
    ...
    static setAllData = (data) => {
        if (data) {
            Store.data = data;
            Store.credits = calcTimeCredits(data);
    ...
    static setYear = (year) => {
        if (Store.loaded && Store.year !== year) {
            Store.year = year;
            Store.holidays = getHolidays(year);
    ...
    static setDateData = (day, month, year, start, pause, end, workPlace) => {
        ...
        Store.credits = calcTimeCredits(Store.data);
